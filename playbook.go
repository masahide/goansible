...
func NewPlaybookBytes(env *Environment, playBytes []byte) (*Playbook, error) {
	...
	if err := yamlBytes(playBytes, &seq); err != nil {
		return nil, err
	}
	plays, err := pb.LoadPlayDatas(seq, pb.Vars)
	if err != nil {
		return nil, err
	}
...
func (pb *Playbook) LoadPlayDatas(seq []playData, s Scope) ([]*Play, error) {
	...
	for _, item := range seq {
		if item.Include != "" {
			...
			play, err := parsePlay(pb.Env, s, pb.baseDir, &item)
...
func (p *Play) importTasks(env *Environment, tasks *Tasks, s Scope, tds []TaskData) error {
	for _, x := range tds {
...
func (p *Play) decodeTasksFile(env *Environment, tasks *Tasks, s Scope, td TaskData) error {
	...
	path, err := ExpandTemplates(s, parts[0])
	if err != nil {
		return err
	}
...
func parsePlay(env *Environment, s Scope, dir string, m *playData) (*Play, error) {
	var play Play
	...
	if len(m.Tasks) > 0 {
		err := play.importTasks(env, &play.Tasks, s, m.Tasks)
		if err != nil {
			return nil, err
		}
	}
	...
	if len(m.Handlers) > 0 {
		err := play.importTasks(env, &play.Handlers, s, m.Handlers)
		if err != nil {
			return nil, err
		}
	}
